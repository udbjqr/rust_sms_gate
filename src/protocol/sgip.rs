...
use crate::protocol::names::{LOGIN_NAME, PASSWORD, NODE_ID, MSG_TYPE_U32, SEQ_ID, STATUS, MSG_CONTENT, SERVICE_ID, SP_ID, SRC_ID, DEST_IDS, SEQ_IDS, TP_UDHI, MSG_FMT, DEST_ID, STATE, RESP_NODE_ID, ERROR_CODE, RESP_SEQ_ID};
...
impl ProtocolImpl for Sgip {
	...
	fn encode_submit(&self, json: &mut JsonValue) -> Result<BytesMut, Error> {
		...
		for i in 0..sms_len {
			...
			dst.put_u32(node_id);
			let mut seq_id = (get_time() << 32) as u64 | (get_sequence_id(dest_ids.len() as u32)) as u64;
			dst.put_u64(seq_id);
	...
	fn encode_report(&self, json: &mut JsonValue) -> Result<BytesMut, Error> {
		...
		dst.put_u32(node_id);
		let seq_id = (get_time() << 32) as u64 | get_sequence_id(1) as u64;
